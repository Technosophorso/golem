...
export class DurableIntakeQueue {
  ...
  enqueue({ definitionKey, idempotencyKey, body, visitorId, continuation }) {
    ...
    if (!body || typeof body !== 'object' || Array.isArray(body) || !body.fields || typeof body.fields !== 'object' || Array.isArray(body.fields)
      || Object.keys(body).some((key) => !['fields', 'attachments', 'externalIdentity', 'submittedAt', 'identityProof'].includes(key))) fail('invalid_submission_body')
    ...
    if (Buffer.byteLength(payload) + Buffer.byteLength(continuationPayload ?? '') > 1_600_000) fail('payload_too_large', 413)
