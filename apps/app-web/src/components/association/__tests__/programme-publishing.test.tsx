...
const api=vi.hoisted(()=>({module:vi.fn(),read:vi.fn(),save:vi.fn(),publish:vi.fn(),confirm:vi.fn()}));
vi.mock('@/lib/api/association',async original=>({...await original<typeof import('@/lib/api/association')>(),getAssociationModuleSnapshot:api.module,getProgrammeCatalogueDraft:api.read,saveProgrammeCatalogueDraft:api.save,publishProgrammeCatalogue:api.publish}));
