...
const control = 'min-h-11 rounded-md border px-3 py-2 text-sm disabled:opacity-50';
...
export function FeedLearnedDecisions(props: {
  ...
  const sourceLinks = (ids: string[], confirmationId: string, revoked: boolean) => (
        <ul className="mt-2 space-y-2 text-xs">{ids.map(id => { const source = props.data?.sources.find(source => source.id === id); return source ? <li key={id} className="break-words"><span>{t.source}: {source.actorName ?? t.unknownAuthor}{source.revision !== null ? ` · ${t.revision} ${source.revision}` : ''}</span><div className="flex flex-wrap gap-2">{source.sessionId === props.sessionId && source.threadId ? <button className={control} onClick={() => props.onThread(source.threadId!)}>{t.openSource}</button> : source.sessionId ? <a className={control} href={feedPostPath(props.workspaceId, props.platform, source.sessionId)}>{t.openSource}</a> : null}{source.canRetract && !revoked ? <button className={control} disabled={unavailable} onClick={() => void governed(confirmationId, { action: 'retractSource', eventId: source.id }, t.retract)}>{t.retract}</button> : null}</div></li> : null; })}</ul>
  );
  return <section className="min-w-0 space-y-3 rounded-lg border p-3" aria-label={t.title}>
    ...
    <button className={control} disabled={unavailable || !props.data?.canConfirm || Boolean(current) || props.unfinished} onClick={async () => { if (await confirmDialog({ title: t.confirm, description: t.confirmExplanation, confirmLabel: t.confirm, cancelLabel: tc.cancel })) await props.actions.confirm(props.reviewRunId); }}>{t.confirm}</button>
    ...
    {props.error || props.actions.error ? <p role="alert" className="text-sm">{t.failed}<button className={control} disabled={props.offline || props.actions.busy} onClick={props.onRefresh}>{tc.retry}</button></p> : null}
    ...
    {editor ? <form className="space-y-2 rounded-md border p-2" onSubmit={event => { event.preventDefault(); void saveEditor(); }}>
      ...
      <div className="flex flex-wrap gap-2"><button className={control} disabled={unavailable || !editor.text.trim()} type="submit">{t.save}</button><button className={control} type="button" onClick={() => setEditor(null)}>{tc.cancel}</button></div>
    </form> : null}
    {props.data?.confirmations.map(item => <article key={item.id} className="min-w-0 space-y-3 border-t pt-3" data-feed-confirmation={item.id}>
      ...
      {item.summary ? <div className="space-y-2"><h4 className="text-sm font-medium">{t.summary}</h4><p className="whitespace-pre-wrap break-words text-sm">{item.summary.text}</p>
        ...
        {item.summary.canEdit && !item.revoked ? <div className="flex flex-wrap gap-2"><button className={control} disabled={unavailable} onClick={() => void governed(item.id, { action: 'scopeSummary', scope: item.summary!.postOnly ? 'future' : 'post' }, item.summary!.postOnly ? t.futureReference : t.onlyThisPost)}>{item.summary.postOnly ? t.futureReference : t.onlyThisPost}</button><button className={control} disabled={unavailable} onClick={() => setEditor({ confirmationId: item.id, kind: 'editSummary', text: item.summary!.text, detail: item.summary!.correction ?? '' })}>{t.edit}</button><button className={control} disabled={unavailable} onClick={() => void governed(item.id, { action: 'forgetSummary' }, t.forget)}>{t.forget}</button></div> : null}
      ...
      {props.data?.canConfirm && !item.revoked ? <button className={control} disabled={unavailable} onClick={() => setEditor({ confirmationId: item.id, kind: 'remember', text: '', detail: '' })}>{t.remember}</button> : null}
      {item.artifacts.map(artifact => <details key={artifact.id} className="min-w-0 rounded-md border p-2" data-feed-artifact={artifact.id}>
        ...
        {artifact.canEdit && !item.revoked ? <div className="mt-2 flex flex-wrap gap-2">
          {!['retired', 'rejected', 'forgotten'].includes(artifact.status) ? <button className={control} disabled={unavailable} onClick={() => setEditor({ confirmationId: item.id, kind: artifact.kind === 'voice' ? 'editVoice' : 'editRule', id: artifact.id, text: artifact.text, detail: '' })}>{t.edit}</button> : null}
          {artifact.kind === 'rule' && artifact.status === 'suggested' ? <button className={control} disabled={unavailable} onClick={() => void governed(item.id, { action: 'decideRule', ruleId: artifact.id, decision: 'approve' }, t.approve)}>{t.approve}</button> : null}
          {artifact.kind === 'rule' && ['retired','rejected'].includes(artifact.status) ? <button className={control} disabled={unavailable} onClick={() => void governed(item.id, { action: 'decideRule', ruleId: artifact.id, decision: 'restore' }, t.restore)}>{t.restore}</button> : null}
          {artifact.kind === 'rule' && ['active','suggested'].includes(artifact.status) ? <button className={control} disabled={unavailable} onClick={() => void governed(item.id, { action: 'decideRule', ruleId: artifact.id, decision: 'dismiss' }, t.dismiss)}>{t.dismiss}</button> : null}
          {artifact.status !== 'forgotten' ? <button className={control} disabled={unavailable} onClick={() => void governed(item.id, artifact.kind === 'rule' ? { action: 'decideRule', ruleId: artifact.id, decision: 'forget' } : { action: 'forgetVoice', memoryId: artifact.id }, t.forget)}>{t.forget}</button> : null}
          {artifact.canPromote && artifact.kind === 'rule' && artifact.status === 'active' ? <button className={control} disabled={unavailable} onClick={() => void governed(item.id, { action: 'scopeRule', ruleId: artifact.id, scope: 'brand_voice' }, t.promote)}>{t.promote}</button> : null}
          {artifact.kind === 'voice' && artifact.status === 'active' ? <button className={control} disabled={unavailable} onClick={() => void governed(item.id, { action: 'scopeVoice', memoryId: artifact.id, scope: 'member' }, t.narrow)}>{t.narrow}</button> : null}
        </div> : null}
          {artifact.status === 'active' && props.data?.canConfirm && !item.revoked ? <button className={control} disabled={unavailable} onClick={() => void governed(item.id, artifact.kind === 'rule' ? { action: 'scopeRule', ruleId: artifact.id, scope: 'post' } : { action: 'scopeVoice', memoryId: artifact.id, scope: 'post' }, t.postOnly)}>{t.postOnly}</button> : null}
        {sourceLinks(artifact.sourceEventIds, item.id, item.revoked)}
      </details>)}
      {!item.revoked && props.data?.canConfirm ? <button className={control} disabled={unavailable} onClick={() => void governed(item.id, { action: 'revoke' }, t.revoke)}>{t.revoke}</button> : null}
    </article>)}
  </section>;
}
