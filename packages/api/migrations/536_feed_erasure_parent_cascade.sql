-- Parent deletion may reach provenance before other native FK cascades.
BEGIN;
...
BEGIN
  ...
  DELETE FROM feed_learning_outputs WHERE session_id=ANY(p_sessions);
  DELETE FROM memories WHERE id=ANY(memory_ids);
  DELETE FROM assistant_playbook_rules WHERE id=ANY(rule_ids) AND NOT EXISTS(SELECT 1 FROM assistants a WHERE a.id=assistant_playbook_rules.assistant_id);
  ...
  UPDATE feed_editorial_runs SET result='{"parts":{},"sourceErased":true}',usage='{}',status='cancelled',lease_id=NULL,lease_until=NULL,
    ...
    WHERE session_id=ANY(p_sessions) AND kind IN ('confirmation_learning','reconcile')
      AND EXISTS(SELECT 1 FROM sessions s WHERE s.id=feed_editorial_runs.session_id);
END $$;
COMMIT;
