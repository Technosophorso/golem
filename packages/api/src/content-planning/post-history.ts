...
export async function readFeedPostHistory(client: FeedReader, input: { workspaceId: string; assistantIds: string[]; sessionId: string; text: string; cursor?: number; additionalHistorySql?: string }) {
  ...
  const openRows = `SELECT d.id::text,d.platform,d.status,coalesce(d.final_text,d.draft_text,'') AS body,d.post_format,jsonb_build_object('threadSegments',d.format_data->'threadSegments','article',d.format_data->'article') AS format_data,coalesce(d.resolved_at,d.created_at) AS date,d.posted_permalink AS link FROM content_planning_drafts d JOIN assistants a ON a.id=d.assistant_id WHERE a.workspace_id=$1 AND d.assistant_id=ANY($2::uuid[]) AND d.platform<>'email' AND COALESCE(d.format_data->'feedCanonical'->'content'->>'sourceSensitivity','public')='public' AND feed_draft_audience_allowed(d.session_id) AND d.status IN ('ready','posted') AND d.removed_at IS NULL AND d.session_id IS DISTINCT FROM $3::uuid`
  const base = `FROM (SELECT * FROM (${openRows}${input.additionalHistorySql ? ` UNION ALL ${input.additionalHistorySql}` : ''}) combined WHERE platform<>'email') d`
