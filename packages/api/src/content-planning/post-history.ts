...
export async function readFeedPostHistory(client: FeedReader, input: { workspaceId: string; assistantIds: string[]; sessionId: string; text: string; cursor?: number }) {
  const base = `FROM content_planning_drafts d JOIN assistants a ON a.id=d.assistant_id WHERE a.workspace_id=$1 AND d.assistant_id=ANY($2::uuid[]) AND d.status IN ('ready','posted') AND d.removed_at IS NULL AND d.session_id IS DISTINCT FROM $3::uuid`
  ...
  const columns = `d.id,d.platform,d.status,coalesce(d.final_text,d.draft_text,'') AS body,d.post_format,d.format_data,coalesce(d.resolved_at,d.created_at) AS date,d.posted_permalink AS link`
  const recent = (await client.query(`SELECT ${columns} ${base} ORDER BY coalesce(d.resolved_at,d.created_at) DESC,d.id DESC LIMIT $4`, [...values, FEED_EDITORIAL_LIMITS.recentPosts])).rows
  const excluded = recent.map(row => row.id)
  const older = terms.length ? (await client.query(`SELECT ${columns} ${base} AND NOT(d.id=ANY($4::uuid[])) AND (to_tsvector('simple',coalesce(d.final_text,d.draft_text,'')) @@ websearch_to_tsquery('simple',$5) OR EXISTS (SELECT 1 FROM unnest($6::text[]) term WHERE strpos(lower(coalesce(d.final_text,d.draft_text,'')),term)>0)) ORDER BY coalesce(d.resolved_at,d.created_at) DESC,d.id DESC LIMIT $7 OFFSET $8`, [...values, excluded, phrase, terms, FEED_EDITORIAL_LIMITS.olderPosts + 1, cursor])).rows : []
