...
export type FeedEditorialModel = {
  model: string; tier: string; inputCharacters: number; maxTokens: number;
...
export function createFeedEditorialModelResolver(options: { provider: LLMProvider; configuredProviders: ProviderAvailability; resolveWorkspaceCustomLlm?: WorkspaceCustomLlmResolver; usageStore?: UsageStore; checkCreditBudget?: CreditBudgetGate }): FeedEditorialModelResolver {
  return async (actor, tier) => {
    ...
    return { model, tier: effectiveTier, inputCharacters, maxTokens, async call(input) {
