...
describe('[COMP:office/capabilities] Office capability and admission contract', () => {
  it('declares every editable path and a reason for every rejection', () => {
    ...
    expect(officeCapabilityManifest.capabilities.filter((capability) => capability.family === 'spreadsheet' && capability.disposition === 'rejected').map((capability) => capability.id)).toEqual(expect.arrayContaining(['spreadsheetChart', 'spreadsheetHyperlink', 'spreadsheetRichText', 'spreadsheetFilter', 'spreadsheetProtection']))
    ...
    expect(officeCapabilityManifest.capabilities.filter((capability) => capability.disposition === 'editable' && capability.assistantAuthoring === 'command')).toHaveLength(36)
