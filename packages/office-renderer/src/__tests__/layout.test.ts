...
describe('[COMP:office/layout] Deterministic Office layout', () => {
  ...
  it('measures and renders exact, multiple and at-least spacing including empty cell paragraphs', () => {
    ...
    expect(svg).toContain('--office-run-line-height:0')
    expect(svg).toContain('line-height:var(--office-run-line-height,inherit)')
    expect(officeParagraphCss({ lineSpacingPt: 14, lineSpacingRule: 'atLeast' })).not.toContain('--office-run-line-height:0')
    expect(officeParagraphCss({ lineSpacingMultiple: 1.5 })).not.toContain('--office-run-line-height:0')
